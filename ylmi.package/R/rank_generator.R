...
rank_generator <- function(dfList, country_list, bygender = FALSE, lastyear = 2009) {
  ...
  index <- lapply(dfList, filter_helper, index = country_list, bygender = bygender, lastyear = lastyear) %>%
    reduce(full_join, by = "ref_area.label")

  index <- full_join(index, country_list, by = "ref_area.label")
  ...
  index <- index %>%
    mutate_at(c("neet", "mismatch", "workingpov", "underemp", "informal", "vulnerable", "elementary", "saff", "nosecondary"), rescale) %>%
  ...
  rank <- index %>%
    mutate(rank = ifelse(rowSums(is.na(.[15:17]))==0, rowMeans(.[15:17], na.rm = TRUE),NA)) %>%
    filter(!is.na(rank)) %>%
    select(country, country_code, rank)
