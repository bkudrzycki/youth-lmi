...
impute_helper <- function(index) {
  ...
  if (sum(!is.na(index$neet)) > 0 & sum(!is.na(index$relative_wc)) > 0 & sum(!is.na(index$mismatch)) > 0) {

  index <- index %>%
    add_column(transition_pct = rowMeans(find_pct(index[2:4]), na.rm = TRUE)) %>% ## avg percentile for transition index
  ...
  mutate(neet = ifelse(rowSums(is.na(index[2:4]))<2, coalesce(neet, neet_impute), neet),
         relative_wc = ifelse(rowSums(is.na(index[2:4]))<2, coalesce(relative_wc, relative_wc_impute), relative_wc),
         mismatch = ifelse(rowSums(is.na(index[2:4]))<2, coalesce(mismatch, mismatch_impute), mismatch))

  }

  if (sum(!is.na(index$workingpov)) > 0 & sum(!is.na(index$underemp)) > 0 & sum(!is.na(index$informal)) > 0 & sum(!is.na(index$elementary)) > 0) {

    index <- index %>%
      add_column(workCond_pct = rowMeans(find_pct(index[5:8]), na.rm = TRUE)) %>% ## repeat for remaining two dimensions
    ...
    index <- index %>%
      mutate(workingpov = ifelse(rowSums(is.na(index[5:8]))<3, coalesce(workingpov, workPov_impute), workingpov),
             underemp = ifelse(rowSums(is.na(index[5:8]))<3, coalesce(underemp, underemp_impute), underemp),
             informal = ifelse(rowSums(is.na(index[5:8]))<3, coalesce(informal, informal_impute), informal),
             elementary = ifelse(rowSums(is.na(index[5:8]))<3, coalesce(elementary, elementary_impute), elementary))

  }

  if (sum(!is.na(index$nosecondary)) > 0 & sum(!is.na(index$literacy)) > 0 & sum(!is.na(index$test_scores)) > 0) {
